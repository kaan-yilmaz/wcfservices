...
{
    public class GetBookByIsbno : IDisposable
    {
        public void Dispose()
        {
        }
        ...
        public GetBookByIsbno(BookStoreEntities bookStoreEntitys)
        {
            this.dbContext = bookStoreEntitys;
        }

        public Book Handle(int isbno)
        {
            var book = dbContext.books.SingleOrDefault(item => item.isbn == isbno);
            if (book == null)
            {
                return null;
            }
