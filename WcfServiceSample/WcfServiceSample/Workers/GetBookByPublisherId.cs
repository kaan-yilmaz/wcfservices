...
{
    class GetBookByPublisherId : IDisposable
    {
        public void Dispose()
        {
        }
        ...
        public GetBookByPublisherId(BookStoreEntities bookStoreEntitys)
        {
            this.dbContext = bookStoreEntitys;
        }

        public List<Book> Handle(int publisherId)
        {
            var books = dbContext.books.Where(item => item.publisher_id == publisherId).ToList();
